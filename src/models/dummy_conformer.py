...
class Rearrange021(nn.Module):
    def forward(self, x):
        return x.permute(0, 2, 1)
   
    
def get_max_value(dtype: torch.dtype):
...
class Attention(nn.Module):
    ...
    def forward(self, x, context: torch.Tensor = torch.zeros(1), mask: torch.Tensor = torch.zeros(1), context_mask: torch.Tensor = torch.zeros(1)):
        ...
        if exists(mask) or exists(context_mask):
            ...
            context_mask = (
                ...
                if not has_context
                    ...
                    context_mask, torch.ones(context.shape[:2], device=device)
                )
            )
            '''replace'''
            ...
            # )
            '''with'''
            mask_value = get_max_value(dots.dtype)
            mask = mask.unsqueeze(1).unsqueeze(-1) * context_mask.unsqueeze(1).unsqueeze(2)
            ''''''
...
class ConformerBlock(nn.Module):
    ...
    def forward(self, x, mask: torch.Tensor = torch.zeros(1)):
        x = self.ff1(x) + x
