from models.dummy_conformer import ConformerBlock
import torch
...
class DilatedDenseNet(nn.Module):
    def __init__(self, depth=4, in_channels=64):
        ...
        for i in range(self.depth):
            ...
            # self.modulels.append(nn.ConstantPad2d((1, 1, pad_length, 0), value=0.0))
            # self.modulels.append(nn.Conv2d(self.in_channels * (i + 1), self.in_channels, kernel_size=self.kernel_size, dilation=(dil, 1)))
            # self.modulels.append(nn.InstanceNorm2d(in_channels, affine=True))
            # self.modulels.append(nn.PReLU(self.in_channels))
            # self.prelus.append(nn.LeakyReLU())
            self.add_module(f'pad{i+1}', nn.ConstantPad2d((1, 1, pad_length, 0), value=0.0))
            self.add_module(f'conv{i+1}', nn.Conv2d(self.in_channels * (i + 1), self.in_channels, kernel_size=self.kernel_size, dilation=(dil, 1)))
            self.add_module(f'norm{i+1}', nn.InstanceNorm2d(in_channels, affine=True))
            self.add_module(f'prelu{i+1}', nn.PReLU(self.in_channels))
    
    def forward(self, x):
        skip = x
        # for i in range(self.depth):
        #     out = self.get_submodule("pad{}".format(i + 1))(skip)
        #     out = self.get_submodule("conv{}".format(i + 1))(out)
        #     out = self.get_submodule("norm{}".format(i + 1))(out)
        #     out = self.get_submodule("prelu{}".format(i + 1))(out)
...
class TSCNet(nn.Module):
    ...
    def forward(self, x):
        ...
        real = x[:, 0, :, :]
        imag = x[:, 1, :, :]
        noisy_phase = torch.where(real > 0,\
            torch.atan(imag / real), torch.sign(imag)\
            * (math.pi - torch.atan(imag / -real))).unsqueeze(1)
        ''''''
